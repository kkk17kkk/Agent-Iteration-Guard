...
function ProjectHeader({ displayName, manifest, baseline, latest, runtime, status, onDetail }) {
  return <section className="project-header-card">
    <div className="project-header-copy">
      <span className="object-type">Project</span>
      <h2>{displayName}</h2>
      <p>{projectPurpose(manifest, displayName)}</p>
    </div>
    <div className="project-header-meta aig-trajectory" aria-label="版本演进轨迹">
      <div className="trajectory-node trajectory-baseline"><span>Baseline</span><strong title={baseline?.baseline_version}>{short(baseline?.baseline_version, 20)}</strong></div>
      <div className="trajectory-node trajectory-candidate"><span>Candidate</span><strong title={latest?.version}>{short(latest?.version, 20)}</strong></div>
...
function ProgressCard({ steps }) {
  ...
  return <section className="dashboard-card progress-card">
    <div className="card-heading-row"><div><h2>评估进度</h2><span className="section-heading-label">Evaluation progress</span></div><strong className="progress-percent">{current >= 0 ? Math.max(1, Math.round(((current + 0.5) / steps.length) * 100)) : percent}%</strong></div>
...
export default function Overview({ projectId, intelligence, scans, uploads, benchmarks = [], reportList = [], report, reportView, gate, onNew, onDetail, onExitProject, onReport, onUploaded, onProjectIdChange, request, refreshProject, setLoading, setNotice, demoMode = false, onDemoLoad }) {
  ...
  return <Page title="总览" kicker="项目工作台 · OVERVIEW" intro="按项目扫描、能力变化与评估证据查看当前状态。" action={<div className="overview-page-actions"><button className="secondary" onClick={onExitProject}><I name="x" />退出项目<I name="arrowRight" /></button><button className="primary" onClick={() => onNew(null)}><I name="flask" />新建评估</button></div>}>
    ...
    <div className="overview-top-grid">
      <section className="dashboard-card status-hero-card"><div className="status-hero-copy"><span className="object-type">Release decision</span><strong className={`hero-status status-word-${status.toLowerCase()}`}>{status}</strong><p>{status === "BLOCKED" ? "当前版本暂不可通过评估" : status === "PASS" ? "最近一次评估已通过当前检查" : "项目已完成扫描，等待下一步评估"}</p><div className="hero-facts"><span><I name="branch" />{short(baseline.baseline_version, 18)} → {short(latest.version, 18)}</span><span><I name="server-gear" />{runtime.runtime_kind || "-"}</span></div></div><div className="decision-index"><span>Gate</span><strong>{latestReport ? "Latest evaluation" : "Awaiting evidence"}</strong></div></section>
